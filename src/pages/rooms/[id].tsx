...
export default function RoomQAID() {
    ...
    useEffect(() => {
        if (roomId === undefined) return // - Prevent undefined in load

        if (!roomExists.loaded) return
        if (roomExists.room) {
            setLoad({ roomExists: true, roomOpen: false, authorizedEntry: true })
        ...
        } else setLoad({ roomExists: false, _404: true, roomOpen: false, authorizedEntry: true })
    }, [ roomId, roomExists, addToast ])
